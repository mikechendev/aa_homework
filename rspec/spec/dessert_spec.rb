...
describe Dessert do
  let(:chef) { double("chef", name: "Yung Dab") }
  ...
  describe "#eat" do
    ...
    it "raises an error if the amount is greater than the quantity" do
      expect { brownie.eat(53)}.to raise_error("not enough left!")
    end
  end

  describe "#serve" do
    it "contains the titleized version of the chef's name" do
      allow(chef).to receive(:titleize).and_return('Chef Yung Dab the Great Baker')
      expect(brownie.serve).to eq('Chef Yung Dab the Great Baker has made 50 brownies!')
    end
  end
